  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onClick}
        className="inline-flex cursor-pointer items-center gap-1 rounded-md border border-border-default bg-white/4 px-1.5 py-0.5 font-mono text-[0.8rem] text-text-soft transition-all hover:border-cyan/35 hover:bg-cyan/[0.07] hover:text-cyan/80"
      >
        {linkCount}
              ...
              <li key={b.id} className="flex gap-1.5 text-[0.78rem] leading-snug">
                ...
                <div className="flex flex-col">
                  ...
                  {(b.authorIds?.length ?? 0) > 0 && (
                    <span className="text-[0.7rem] text-text-dimmed">{bookAuthorDisplay(b, authorsMap)}</span>
                  )}
                </div>
              </li>
            ))}
