...
export default function AuthorsPanel({ open, onClose, nodes, selectedAuthor, onSelectAuthor }) {
    ...
    >
      <div className="h-full overflow-y-auto px-4 pb-6 pt-4">
        ...
        <div className="space-y-1">
          {filtered.length === 0 ? (
            <p className="px-3 py-3 text-[0.8rem] text-white/40">Aucun·e auteur·ice trouvé·e.</p>
          ) : (
            filtered.map((a) => {
              ...
              return (
                <div key={a.name}>
                  <button
                    type="button"
                    className={[
                      'flex w-full cursor-pointer items-center justify-between gap-3 rounded-lg border px-3 py-2.5 text-left backdrop-blur-xl transition-all',
                      isSelected
                        ? 'border-[rgba(255,180,130,0.35)] bg-[rgba(255,180,130,0.12)] hover:bg-[rgba(255,180,130,0.18)]'
