...
export default function DuplicateWarning({ possibleDuplicates }) {
  ...
  return (
    <div className="rounded-[10px] border border-[rgba(255,204,0,0.3)] bg-[rgba(255,204,0,0.06)] px-4 py-3">
      <p className="mb-2 text-[0.78rem] font-semibold text-[rgba(255,204,0,0.9)]">
        &#9888; Doublon possible d&eacute;tect&eacute;
      </p>
      <ul className="flex list-none flex-col gap-1.5">
        {possibleDuplicates.map((n) => (
          <li key={n.id} className="flex items-center gap-2 text-[0.8rem] text-white/60">
            ...
            />
            <span>
              <strong className="text-white/80">{n.title}</strong> {' '}&mdash; {n.author}, {n.year}
            </span>
          </li>
        ))}
      </ul>
